...
def matches_from_puuid(puuid):
    ...
    for name in names:
        try:
            ...
            soloFivesData= summoner.league_entries[Queue.ranked_solo_fives].to_json()
    ...
    allSoloFivesInfo.sort(key=lambda x: (rankedTiers.index(x['tier']), rankedRanks.index(x.rank), x.leaguePoints))
    return allSoloFivesInfo